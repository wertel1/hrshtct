...
cur = conn.cursor()
cur.execute('CREATE TABLE IF NOT EXISTS entries (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, date TEXT, mood INTEGER, work REAL, sleep REAL, comment TEXT)')
conn.commit()
...
def stats_kb():
    ...
    kb.add(InlineKeyboardButton("Неделя", callback_data="s_week"), InlineKeyboardButton("Месяц", callback_data="s_month"), InlineKeyboardButton("Инсайты", callback_data="s_insights"))
    return kb
...
def save(user_id, mood, work, sleep, comment):
    cur.execute("INSERT INTO entries (user_id, date, mood, work, sleep, comment) VALUES (?,?,?,?,?,?)", (user_id, datetime.now().strftime('%Y-%m-%d'), mood, work, sleep, comment))
    conn.commit()
...
def start(m):
    bot.send_message(m.chat.id, "Трекер настроения.\n/add - записать день", reply_markup=main_kb())
...
def comment(m):
    uid = m.chat.id
    d = data[uid]
    comment = '' if m.text == 'Пропустить' else m.text
    save(uid, d['mood'], d['work'], d['sleep'], comment)
    del data[uid]
    bot.send_message(uid, f" Сохранено!\nНастроение: {d['mood']}/5\nРабота: {d['work']}ч\nСон: {d['sleep']}ч", reply_markup=main_kb())
...
def settings(m):
    ...
    bot.send_message(m.chat.id, " Настройки", reply_markup=kb)
